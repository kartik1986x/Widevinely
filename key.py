...
from pywidevinely.utils.protos.license_protocol_pb2 import License


class Key:
    def __init__(
        self, type_: str, kid: UUID, key: bytes, permissions: Optional[list[str]] = None
    ):
        self.type = type_
        self.kid = kid
        self.key = key
    ...
    def from_key_container(cls, key: License.KeyContainer, enc_key: bytes) -> Key:
        ...
        return Key(
            ...
            key=Padding.unpad(
                AES.new(enc_key, AES.MODE_CBC, iv=key.iv).decrypt(key.key), 16
            ),
