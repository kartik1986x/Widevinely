...
class _Structures:
    ...
    v2 = Struct(
        ...
        "type_" / CEnum(Int8ub, **{t.name: t.value for t in _Types}),
...
class Device:
    ...
    def __init__(
        ...
        **__: Any,
        ...
        if not client_id:
            log.exit(
                "Client ID is required, the WVD file does not contain one or is malformed."
            )
        if not private_key:
            log.exit(
                "Private Key is required, the WVD file does not contain one or is malformed."
            )
        ...
        try:
            ...
            log.exit(f"Failed to parse client_id as a ClientIdentification, {e}")
        ...
        if self.client_id.vmp_data:
            try:
                ...
                log.exit(f"Failed to parse Client ID's VMP data as a FileHashes, {e}")
        ...
        try:
            ...
            log.exit(
                f"Failed to parse the Signed DRM Certificate of the Client ID, {e}"
            )
        ...
        try:
            ...
            log.exit(f"Failed to parse the DRM Certificate of the Client ID, {e}")
    ...
    def loads(cls, data: Union[bytes, str]) -> Device:
        ...
        if not isinstance(data, bytes):
            log.exit(f"Expecting Bytes or Base64 input, got {data!r}")
        return cls(**cls.supported_structure.parse(data))

    @classmethod
    def load(cls, path: Union[Path, str]) -> Device:
        if not isinstance(path, (Path, str)):
            log.exit(f"Expecting Path object or path string, got {path!r}")
    ...
    def dumps(self) -> bytes:
        ...
        return self.supported_structure.build(
            dict(
                ...
                client_id_len=len(self.client_id.SerializeToString())
                if self.client_id
                else 0,
                client_id=self.client_id.SerializeToString()
                if self.client_id
                else None,
            )
        )
    ...
    def migrate(cls, data: Union[bytes, str]) -> Device:
        ...
        if header.version == 2:
            log.exit("Device Data is already migrated to the latest version.")
        if header.version == 0 or header.version > 2:
            ...
            log.exit("Device Data does not seem to be a WVD file (v0).")
        ...
        if header.version == 1:  # v1 to v2
            ...
            if data.vmp:
                try:
                    ...
                    log.exit(f"Failed to parse VMP data as FileHashes, {e}")
                data.vmp = vmp
                ...
                if client_id.vmp_data and client_id.vmp_data != new_vmp_data:
                    logger.getLogger("migrate").warning(
                        "Client ID already has Verified Media Path data"
                    )
            ...
            try:
                ...
                log.exit(f"Migration failed, {e}")

        try:
            return cls.loads(data)
        except ConstructError as e:
            log.exit(
                f"Device Data seems to be corrupt or invalid, or migration failed, {e}"
            )
