...
class parse:
    def __init__(self, keybox_path) -> None:
...
class Keybox:
    def __init__(self, data: bytes):
        ...
        if length not in (128, 132):
            raise ValueError(
                f"Invalid keybox length: {length}. Should be 128 or 132 bytes"
            )
        ...
        if body_crc_expected != body_crc:
            raise ValueError(
                f"Keybox CRC is bad. Expected: 0x{body_crc_expected:08X}. Computed: 0x{body_crc:08X}"
            )
