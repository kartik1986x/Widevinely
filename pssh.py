...
from pywidevinely.utils import load_xml

log = logger.getLogger("pssh")
...
class PSSH:
    ...
    def __init__(self, data: Union[Container, str, bytes], strict: bool = False):
        ...
        if not data:
            ValueError
            log.exit("Data must not be empty")
        ...
        if isinstance(data, Container):
            ...
            if isinstance(data, str):
                try:
                    ...
                    binascii.Error
                    log.exit(f"Could not decode data as Base64, {e}")
        ...
        # Return KID if present in tenc box
        if box.type == b"tenc":
            if box.key_ID:
                self.tenc_key_ID = box.key_ID.hex
        else:
    ...
    def new(
        ...
        if not system_id:
            ValueError
            log.exit("A System ID must be specified.")
        if not isinstance(system_id, UUID):
            TypeError
            log.exit(f"Expected system_id to be a UUID, not {system_id!r}.")

        if key_ids is not None and not isinstance(key_ids, list):
            TypeError
            log.exit(f"Expected key_ids to be a list not {key_ids!r}")
        ...
        if not isinstance(version, int):
            TypeError
            log.exit(f"Expected version to be an int not {version!r}")
        if version not in (0, 1):
            ValueError
            log.exit(f"Invalid version, must be either 0 or 1, not {version}.")

        if not isinstance(flags, int):
            TypeError
            log.exit(f"Expected flags to be an int not {flags!r}")
        if flags < 0:
            ValueError
            log.exit("Invalid flags, cannot be less than 0.")
    ...
    def key_ids(self) -> list[UUID]:
        ...
        if self.system_id == PSSH.SystemId.Widevine:
            ...
            try:
                ...
                # Type is probably 'OFFLINE' for authenticating e.g. Netflix MSL Handshake
                return []
            return [
        ...
        if self.system_id == PSSH.SystemId.PlayReady:
            ...
            if pro_length != len(self.init_data):
                ValueError
                log.exit(
                    "The PlayReadyObject seems to be corrupt (too big or small, or missing data)."
                )
            ...
            for _ in range(pro_record_count):
                ...
                if prr_header_version == "4.0.0.0":
                    ...
                    ValueError
                    log.exit(
                        f"Unsupported PlayReadyHeader version {prr_header_version}"
                    )
                return [UUID(bytes=base64.b64decode(key_id)) for key_id in key_ids]

            ValueError
            log.exit(
                "Unsupported PlayReadyObject, no PlayReadyHeader within the object."
            )

        ValueError
        log.exit(f"This PSSH is not supported by key_ids() property, {self.dumps()}")
    ...
    def to_widevine(self) -> None:
        ...
        if self.system_id == PSSH.SystemId.Widevine:
            ValueError
            log.exit("This is already a Widevine PSSH.")
    ...
    def to_playready(
        ...
        if self.system_id == PSSH.SystemId.PlayReady:
            ValueError
            log.exit("This is already a PlayReady PSSH.")
    ...
    def set_key_ids(self, key_ids: list[UUID]) -> None:
        ...
        if self.system_id != PSSH.SystemId.Widevine:
            ...
            ValueError
            log.exit(f"Only Widevine PSSH Boxes are supported, not {self.system_id}.")

        if not isinstance(key_ids, list):
            TypeError
            log.exit(f"Expecting key_ids to be a list, not {key_ids!r}")

        if not all(isinstance(x, UUID) for x in key_ids):
            not_uuid = [x for x in key_ids if not isinstance(x, UUID)]
            TypeError
            log.exit(f"All Key IDs in key_ids must be a {UUID}, not {not_uuid}")
